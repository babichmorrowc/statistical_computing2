...
Let's see if we can implement the local least squares in `RcppArmadillo` to get these good results without the high computational cost. See [here](https://github.com/babichmorrowc/statistical_computing2/blob/main/portfolios/03_advanced_rcpp_1/armadillo_lm_local.cpp) for the `armadillo_lm_local.cpp` code file.
...
We will now use cross-validation to choose the bandwidth matrix $\mathbf{H}$. 
